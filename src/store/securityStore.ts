    ...
    issues: [
      {
        id: '5-1',
        title: 'Reused Password',
        description: 'This password is identical to your email account password',
        severity: 'high',
        fixed: false,
      },
      {
        id: '5-2',
        title: 'No 2FA Enabled',
        description: 'Two-factor authentication is not active on this financial account',
        severity: 'high',
        fixed: false,
      },
      {
...
export const calculateSecurityScore = (accounts: Account[]): number => {
  ...
  const moderateAccounts = accounts.filter(a => a.riskLevel === 'moderate').length;
  const unsafeAccounts = accounts.filter(a => a.riskLevel === 'unsafe').length;

  // Calculate base score
  let score = (safeAccounts * 100 + moderateAccounts * 70 + unsafeAccounts * 30) / totalAccounts;
  ...
  return accounts.map(account => {
    if (account.id === accountId) {
      ...
      if (remainingIssues.length === 0) {
        ...
        const hasMediumSeverity = remainingIssues.some(i => i.severity === 'medium');
