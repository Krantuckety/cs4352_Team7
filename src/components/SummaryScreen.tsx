...
export function SummaryScreen({ onBackToDashboard }: SummaryScreenProps) {
  ...
  const totalIssues = accountsData.reduce(
    (acc, account) => acc + account.issues.length,
    0,
  );
  const criticalIssues = accountsData.reduce(
    (acc, account) =>
      acc + account.issues.filter((i) => i.severity === "high").length,
    0,
  );
  const accountsNeedingWork = accountsData.filter(
    (a) => a.status === "needs-work" || a.status === "unsafe",
  ).length;
  ...
  const improvements = [
    ...
    {
      label: "Accounts Needing Work",
      value: accountsNeedingWork.toString(),
      change: `of ${accountsData.length}`,
    },
    {
      label: "Safe Accounts",
      value: accountsData.filter((a) => a.status === "safe").length.toString(),
      change: `${Math.round((accountsData.filter((a) => a.status === "safe").length / accountsData.length) * 100)}%`,
    },
    { label: "Current Score", value: "72", change: "Needs work" },
  ];

  const unsafeAccounts = accountsData.filter(
    (a) => a.status === "unsafe" || a.status === "needs-work",
  );
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 justify-center">
        ...
        <Button variant="outline" className="px-8 border-2 border-gray-800">
          Download Report
        </Button>
        <Button variant="outline" className="px-8 border-2 border-gray-800">
          Share Summary
