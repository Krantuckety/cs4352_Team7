      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <Card className="border-2 border-gray-800 p-6">
          <div className="text-center">
            <div
              className={`w-20 h-20 rounded ${getRiskColor()} mx-auto mb-4`}
            />
            <h3 className="text-xl font-bold mb-2">{account.name}</h3>
            <div
              className={`inline-block px-4 py-2 ${getRiskColor()} text-white font-bold`}
            >
              {getRiskLabel()}
            </div>
          </div>
        ...
        <Card className="border-2 border-gray-800 p-6 col-span-2">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="font-bold text-2xl text-danger">
                {activeIssues.length}
              </div>
            </div>
          </div>
      ...
      {activeIssues.length > 0 ? (
        <div>
          <div className="flex items-center justify-between mb-6 mt-6">
            <h3 className="text-2xl font-bold">
              Security Issues ({activeIssues.length})
            </h3>
            <Button
            ...
            {activeIssues.map((issue) => (
              <Card key={issue.id} className="border-2 border-gray-800 p-6">
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 ${getSeverityColor(issue.severity)} flex items-center justify-center flex-shrink-0 text-white font-bold text-xl`}
                  >
                    !
                  </div>
        ...
        <Card className="border-2 border-safe p-8 text-center">
          ...
          <p className="text-gray-700">
            This account has no security issues. Great job!
          </p>
        </Card>
      )}
          ...
          <div className="space-y-3">
            {activeIssues.some((i) =>
              ...
              <div className="flex items-start gap-3">
                ...
                <p>
                  <strong>Reused passwords</strong> mean if one account is
                  compromised, hackers can access all your other accounts using
                  the same password.
                </p>
              </div>
            )}
            {activeIssues.some((i) =>
              ...
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-danger rounded-full mt-2" />
                <p>
                  <strong>Two-factor authentication (2FA)</strong> adds an extra
                  security layer, making it much harder for attackers to access
                  your account.
                </p>
              </div>
            )}
            {activeIssues.some((i) =>
              ...
              <div className="flex items-start gap-3">
                ...
                <p>
                  <strong>Weak passwords</strong> can be cracked quickly by
                  automated tools. Strong passwords are your first line of
                  defense.
                </p>
              </div>
            )}
